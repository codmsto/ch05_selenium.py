...
# 1. 셀레니움 웹브라우저 설정
options = Options()
options.add_experimental_option("detach",True) # 웹 브라우저 종료 X(개발 완료시 지움)
# options.add_argument("headless") # 셀레니움 백그라운드 동작(개발 완료 때)

# 셀레니움이 로봇이 아닌척 하는 방법 # 중요
...
print(driver.page_source) # -> 정우성 검색 네이버 페이지 소스코드 # 어느시점에서 driver하냐에 따라 달라짐 중요
